...
export default function DurationPicker({
  ...
  const handleTimeChange = (type: "from" | "to", value: string) => {
    ...
    const parsed = parse(
      value,
      "HH:mm",
      type === "from" ? dateRange.from : dateRange.to,
    );
  ...
  return (
    <div className="flex flex-col gap-8 p-6">
      <div className="flex flex-col gap-4">
        ...
        {/* END TIME BLOCK */}
        <div
          className={`group relative flex flex-col gap-3 p-5 rounded-3xl border-2 transition-all ${
            error
              ? "bg-red-50 border-red-200"
              : "bg-secondary/20 border-transparent focus-within:border-lime"
          }`}
        >
          <div className="flex justify-between items-center">
            <span className="text-xs font-bold uppercase text-muted-foreground">
