...
describe("deriveMasterKey", () => {
  test("master key should be 16 bytes (128 bits)", async () => {
    const masterKey = await derive128BitKey({
      passkey: "securePassword123",
      email: "user@example.com",
    });
    expect(masterKey.byteLength).toBe(16);
  });

  test("same inputs produce identical keys", async () => {
    const key1 = await derive128BitKey({
      passkey: "testPass",
      email: "test@example.com",
    });

    const key2 = await derive128BitKey({
      passkey: "testPass",
      email: "test@example.com",
    });
  ...
  test("custom rounds parameter works", async () => {
    const key = await derive128BitKey({
