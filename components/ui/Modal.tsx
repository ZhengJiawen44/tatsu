...
const Modal = ({
  ...
  useEffect(() => {
    const onEscape = (ev: KeyboardEvent) => {
      if (ev.key === "Escape") handleOpenChange(false);
    };
...
const ModalContent = ({
  ...
  return (
    <div
      className={cn(
        "bg-background rounded-lg w-full max-w-lg max-h-[90%] p-6 overflow-hidden   flex flex-col gap-8",
        className,
      )}
      onMouseDown={(e) => e.stopPropagation()}
      onPointerDown={(e) => e.stopPropagation()}
...
const ModalHeader = ({
  ...
  <div
    className={`flex flex-col w-full space-y-1.5 text-center sm:text-left ${className}`}
  >
    {children}
  </div>
);
...
const ModalBody = ({
  ...
  <div
    className={`max-w-4xl w-full mx-auto px-6 pb-28 bg-background h-full overflow-scroll ${className}`}
  >
    {children}
  </div>
);
