...
const ProjectCollapsible = () => {
  ...
  return (
    <div className="group w-full">
      <div className={clsx("w-full items-start justify-start ")}>
        <div
          className={clsx(
            "flex gap-3 items-center pl-5 pr-3! w-full h-11 font-normal hover:bg-inherit cursor-default",
          )}
        >
          <Folder className={clsx("w-5 h-5 stroke-muted-foreground")} />
          ...
          {createLoading ? (
            ...
            >
              <Button
                size={"icon"}
                className="w-fit h-fit p-1 bg-inherit hover:bg-popover-accent cursor-pointer! text-muted-foreground hover:text-foreground hidden group-hover:flex"
              >
        ...
        <div>
          {isPending ? (
            ...
            Object.entries(projectMetaData).map(([key, value]) => {
              return (
                <ProjectSidebarItem key={key} meta={{ id: key, ...value }} />
              );
            })
          )}
        </div>
