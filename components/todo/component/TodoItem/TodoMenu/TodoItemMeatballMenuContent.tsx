...
function TodoItemMeatballMenuContent({
  ...
  const { usePrioritizeTodo, useDeleteTodo, usePinTodo, useEditTodo } =
    useTodoMutation();
  ...
  return (
    <>
      <DropdownMenuItem
        className="mx-1 p-1.5 px-2 gap-1.5"
        onClick={() => pinMutateFn(todo)}
      >
        {!todo.pinned ? <Pin className="m-0" /> : <Unpin />}
        <p className="">
          {todo.pinned ? todayDict("menu.unpin") : todayDict("menu.pinToTop")}
        </p>
      </DropdownMenuItem>
      <DropdownMenuItem
        className="m-1.5"
        ...
        <DropdownMenuSubContent className="max-h-56 overflow-scroll">
          {Object.entries(projectMetaData).map(([key, value]) => {
            ...
            return (
              ...
              >
                <ProjectTag id={key} className="text-base pr-0" />
                {value.name}
              </DropdownMenuItem>
            );
          })}
      ...
      >
        ...
        <div className=" flex gap-4 items-center pl-2">
          ...
          >
            <Flag className="text-lime group-hover:fill-lime" />
          </button>
          ...
          >
            <Flag className="text-orange group-hover:fill-orange" />
          </button>
          <button
            className="group cursor-pointer"
            onClick={() => {
          ...
          >
            <Flag className="text-red group-hover:fill-red" />
          </button>
        </div>
      </DropdownMenuItem>
    </>
