...
export async function POST(req: NextRequest) {
  try {
    ...
    const { title, description, priority, dtstart, due, rrule, projectID } = parsedObj.data
    ...
    const todo = await prisma.todo.create({
      data: {
        ...
        rrule,
        projectID,
        durationMinutes: dtstart && due ? (due?.getTime() - dtstart?.getTime()) / (1000 * 60) : undefined
      }
    })
    ...
    if (todoCalendar && todo.dtstart && todo.due) {
      try {
        ...
        if (!res.ok) {
          throw new Error(`CalDAV server rejected the request: ${res.status} ${res.statusText}`)
        }
    ...
    return NextResponse.json({ message: 'todo created', todo }, { status: 200 })
  } catch (error) {
    console.log(error)
    ...
    return NextResponse.json(
      {
        message: error instanceof Error ? error.message.slice(0, 50) : 'an unexpected error occured'
      },
      { status: 500 }
    )
  }
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!start || !end) throw new BadRequestError('date range start or from not specified')
    ...
    const ghostTodos = expandAndMergeTodos(recurringTodos, timeZone, dateRangeStart, dateRangeEnd)

    //remove ghosts that are either overdue or completed
    const filteredGhosts = ghostTodos.filter(todo => {
      return (!todo.due || todo.due >= dateRangeStart) && todo.completed === false
    })
    const allTodos = [...oneOffTodos, ...filteredGhosts].sort((a, b) => a.order - b.order)
