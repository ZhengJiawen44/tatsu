...
export function genICSData({ summary, description, start, end, rrule }: createICSDataProps) {
  ...
  if (start) {
    event.updatePropertyWithValue('dtstart', ICAL.Time.fromJSDate(start, false))
  }
  if (end) {
